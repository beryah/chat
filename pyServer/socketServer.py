...
handler = logging.handlers.RotatingFileHandler(
    'tsew.log', "a", maxBytes=50000000, backupCount=10)
...
class MyServerProtocol(WebSocketServerProtocol):
    ...
    def onMessage(self, payload, isBinary):
        if isBinary:
            ...
            logger.debug("payload.decode('utf8'): {0}".format(
                payload.decode('utf8')))
...
def get_token(to_as_data, ws):
    register_url = 'http://{0}/api/as/v2/session/register'.format(
        '10.206.132.8')
    rtext = http_request(register_url, to_as_data, desc='get_token')
    data = json.loads(rtext)
    get_token_ready(data, to_as_data['clientId'], to_as_data[
                    'supportId'], 'DLS', ws)
    return data
...
def get_token_ready(data, clientId, supportId, source, ws):
    ...
    threads[key] = {'status': 'running',
                    'first_time': datetime.utcnow(), 'terminate_time': None}
    threading.Thread(target=polling_as, args=(
        token_id, session_id, support_id, client_id, source, ws), name=key).start()
